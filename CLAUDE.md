...
./target/release/siteone-crawler --url=https://example.com --ai-provider=openai-compatible \
  --ai-endpoint=http://localhost:8000/v1 --ai-model=my-model --ai-profile           # chapter-based subject profile (MD+JSON+HTML)
./target/release/siteone-crawler --url=https://example.com --ai-profile --ai-dry-run # plan only, no API calls
```
...
1. **CLI Parsing** (`Initiator` → `CoreOptions::parse_argv()`): Parses 120+ CLI options, merges config file if present, validates. Exits with code 101 on error, code 2 on `--help`/`--version`. Non-crawl utility modes (`--serve-markdown`, `--serve-offline`, `--html-to-markdown`, `--ai-list-models`, `--ai-check`) exit early in `main.rs` before creating the Manager.

2. **Analyzer Registration** (`Initiator::register_analyzers()`): Creates all 18 analyzer instances (Accessibility, BestPractice, BrowserConsole, Caching, ContentType, DNS, ExternalLinks, Fastest, Headers, Page404, Redirects, Security, SeoAndOpenGraph, SkippedUrls, Slowest, SourceDomains, SslTls, Technologies) and registers them with `AnalysisManager`. Some analyzers receive config from CLI options (e.g. `fastest_top_limit`, `max_heading_level`); `BrowserConsole` is active only in `--browser` mode.
...
7. **Exporters** (`Manager::run_exporters()`): Generate output files based on CLI options:
   ...
   - `AiElaborateExporter` (Markdown + JSON + HTML): when `--ai-elaborate` produced a `BrandDoc`, atomically writes a no-clobber triple named `ai-elaborate.<template>.<host>.<run-id>.md` + `.json` + `.html` (rolls back earlier files if a later one fails)
...
- `src/browser/` (feature `browser`): `BrowserRenderer` (renderer.rs), Chromium detection/download/launch (launcher.rs), CDP diagnostics collection (diagnostics.rs), screenshots in every `--screenshot-viewport` size + pre-capture animation settling (screenshot.rs), `--screenshot-viewport` parsing: `WxH`/presets, up to 5 sizes (viewport.rs), pre-capture auto-scroll for lazy-loaded and scroll-triggered content (auto_scroll.rs, `--browser-auto-scroll`, on by default), cookie-banner dismissal/hiding (cookie_consent.rs). `diagnostics.rs` data types are always compiled so `HttpResponse` can carry an inert `Option<BrowserDiagnostics>`, and so is `viewport.rs`, because option validation runs in every build
...
- `src/ai/elaborate/` (`--ai-elaborate`): builds one large, richly structured brand profile (Markdown + JSON + self-contained HTML). Post-crawl pipeline (mirrors `src/ai/summary/`), orchestrated by `mod.rs::run`: `selection.rs::build_candidates` (full ranked universe) → `cluster.rs` (mass-entity URL clustering, e.g. `/blog/*`) + `nav.rs` (global-nav detection) → optional `gapfill.rs` (bounded, robots-honoring, mask-respecting fetch of nav pages the crawl missed) → `select.rs` (two id-constrained LLM rounds: the model only ever returns integer ids into numbered lists, so hallucinated URLs are impossible; a deterministic floor/cluster-rep union guards a weak round) → `extract.rs` (per-page typed `PageEssence`, verbatim entities) → `merge.rs` (deterministic dedup into an un-hallucinatable `BrandModel` — the crawler renders the structured lists, the LLM never does) → `synthesize.rs` (prose-only, single-shot or output-cap-driven sectioned map-reduce) into a fixed per-`templates.rs` skeleton → optional `correct.rs` pass (safe `{from,to}` edits applied to PROSE ONLY, unique-match, never touching verbatim data). Output language reuses `--ai-report-language` + `report::locale::ReportLocale`. `doc.rs` (`BrandDoc`) renders MD/JSON/HTML; failed pages are counted, listed, and excluded — never fabricated. `--ai-elaborate` is its own pipeline (not an `--ai-actions` value): used alone it clears the default actions; combined with explicit `--ai-actions` both run. `src/export/ai_elaborate_exporter.rs` writes the paired MD+JSON+HTML
- `src/ai/profile/` (`--ai-profile`): a second, independent elaborate-style pipeline that builds a polished, chapter-based **subject profile** (MD + JSON + self-contained HTML). Post-crawl (`mod.rs::run`): candidate universe → `summarize.rs` (≤1000-char site description) → `classify.rs` (id-constrained pick of one of 13 subject types; failure → `general`) → `describe.rs` (≤300-char per-page descriptions, parallel) → per-chapter `select.rs` (numbered-id page selection, un-hallucinatable) + `synthesize.rs` (chapter markdown) + `correct.rs` (universal `{from,to}` edits with exact/`regex:`/ellipsis forms and delete guards), all chapters in parallel under `--ai-max-concurrency` → executive summary. Prompt content is embedded from repo-root `prompts/profile/**` via `include_str!` (`registry.rs`/`embedded.rs`, generated by `scripts/gen_profile_prompts.py`; parser in `promptpack.rs`). Input budgets scale from `--ai-context-window` (reference 128K) in `budget.rs`. `doc.rs` (`ProfileDoc`) renders MD/JSON/HTML; `src/export/ai_profile_exporter.rs` writes the no-clobber triple `ai-profile.<type>.<host>.<run-id>.{md,json,html}`. Left the existing `--ai-elaborate` (v1) untouched.
- `src/ai/telemetry.rs` + `src/ai/progress.rs`: every LLM HTTP attempt and cache hit goes through `AiClient::complete_cached` (`src/ai/client.rs`), which builds one `RequestRecord` per attempt (tokens incl. reasoning/cached, send→body time, tok/s) and `telemetry::report`s it: a line on stderr (`  AI ✓ #12 SEO 12/40 · /blog/post · 3,412 in · 812 out (540 reasoning) · 6.8 s · 119 tok/s`, hidden by `--hide-progress-bar`) plus an `aiRequest` event. The page/stage comes from a tokio task-local set by `telemetry::scope`/`progress::unit` in each pipeline; `progress` counts units per task (`seo`, `report:<preset>`, `profile:chapters`, …, one `advance` per unit, failed ones too) for the `12/40` and the `aiProgress` events. Usage is parsed shape-first (`provider::parse_usage`, `parse_reasoning`); an unknown shape only means missing statistics, never a panic
- `src/ai/tools.rs` (`--ai-list-models`, `--ai-check`): one-shot utility modes for the GUI's model picker, run from `main.rs` before any crawl (no `--url`). Each prints exactly one JSON object on stdout, also on failure (`{"ok":false,"error":…}`, exit 1; exit 101 for a configuration error, answered in JSON without the help text) and never a credential. `provider::models_request`/`parse_model_list` shape the model-list GET and read `data[]`/`models[]` (context window from `max_model_len`/`max_input_tokens`/`inputTokenLimit`, …); `--ai-check` sends one prompt through the normal `AiClient` with the cache off
- `src/events.rs` + `docs/EVENTS.md`: the NDJSON event stream of `--events-file` (protocol 1, additive only): run/phase/URL/artifact/issue events and the AI events `aiRequest`, `aiProgress`, `aiUsage`. `docs/EVENTS.md` documents every event with an example line; keep it in step with `events.rs`
