...
- `src/browser/` (feature `browser`): `BrowserRenderer` (renderer.rs), Chromium detection/download/launch (launcher.rs), CDP diagnostics collection (diagnostics.rs), screenshots + pre-capture animation settling (screenshot.rs), pre-capture auto-scroll for lazy-loaded and scroll-triggered content (auto_scroll.rs, `--browser-auto-scroll`, on by default), cookie-banner dismissal/hiding (cookie_consent.rs). `diagnostics.rs` data types are always compiled so `HttpResponse` can carry an inert `Option<BrowserDiagnostics>`
