...
| `--http-auth=<user:pass>` | Basic HTTP authentication in `username:password` format. Sent only to the crawled site (the initial host's<br>registrable domain, e.g. `*.example.co.uk`), never to other domains, and never over plain `http` when the<br>crawl starts on `https`. For an IP address or `localhost`, only the initial port counts as the crawled site.<br>Tip: responses fetched with credentials are stored in the HTTP cache like any other; use `--http-cache-dir=`<br>(empty) to disable caching for authenticated crawls. |
