...
pub struct ProfileMeta {
    ...
    pub calls: usize,
    /// Total input (prompt) tokens the API reported across the profile's LLM calls.
    pub input_tokens: u64,
    /// Total output (completion) tokens the API reported.
    pub output_tokens: u64,
    /// Summed wall time of the individual LLM requests (they run in parallel, so this typically
    /// exceeds `gen_ms`).
    pub llm_ms: u64,
    /// End-to-end wall-clock time of the profile generation pipeline.
    pub gen_ms: u64,
...
impl ProfileDoc {
    ...
    /// A compact one-line LLM statistic for the footer (calls, in/out tokens, summed request time,
    /// wall-clock generation time). None when no LLM calls were made (e.g. dry-run). The summed
    /// request time normally exceeds the generation time because calls run in parallel.
    pub(crate) fn llm_stats(&self, czech: bool) -> Option<String> {
        let m = &self.meta;
        if m.calls == 0 {
            return None;
        }
        if czech {
            Some(format!(
                "LLM: {} volání · vstup {} / výstup {} tokenů · souhrnný čas dotazů {} · generování {} (dotazy běžely paralelně)",
                m.calls,
            ...
            Some(format!(
                "LLM: {} calls · {} in / {} out tokens · request time {} · generated in {} (calls ran in parallel)",
                m.calls,
                fmt_tokens(m.input_tokens),
                fmt_tokens(m.output_tokens),
                fmt_dur_ms(m.llm_ms),
    ...
    pub fn to_markdown(&self) -> String {
        ...
        // Footer: LLM stats + a linked "generated by" credit.
        out.push_str("\n---\n\n");
        if let Some(stats) = self.llm_stats(czech) {
            out.push_str(&format!("*{}*\n\n", stats));
        }
        let generated_by = if czech {
    ...
    pub fn to_json(&self) -> Value {
        json!({
            ...
            "meta": {
                ...
                "inputTokens": self.meta.input_tokens,
                "outputTokens": self.meta.output_tokens,
                "llmMs": self.meta.llm_ms,
                "genMs": self.meta.gen_ms,
...
impl ProfileDoc {
    ...
    pub fn to_html(&self) -> String {
        ...
        h.push_str(&format!(
            "<p class=\"gen\">{} <a href=\"https://crawler.siteone.io/\" target=\"_blank\" rel=\"noopener\">SiteOne Crawler</a> (<a href=\"https://github.com/janreges/siteone-crawler\" target=\"_blank\" rel=\"noopener\">GitHub</a>).</p></footer>\n",
            generated_by
        ));
        h.push_str("<script>\n");
        h.push_str(JS);
...
/// Compact human token count for the footer: "1.9M", "31.2k", "742".
...
/// Human duration for the footer: "6 min 3 s" for a minute or more, else "12.4 s".
...
mod tests {
    ...
    #[test]
    fn footer_has_links_and_llm_stats() {
        let mut d = sample();
        d.meta.calls = 164;
        d.meta.input_tokens = 1_900_000;
        d.meta.output_tokens = 31_200;
        d.meta.llm_ms = 492_000; // 8 min 12 s
        d.meta.gen_ms = 363_000; // 6 min 3 s
        for out in [d.to_markdown(), d.to_html()] {
            assert!(out.contains("https://crawler.siteone.io/"), "crawler link present");
            assert!(
                out.contains("https://github.com/janreges/siteone-crawler"),
                "github link present"
            );
            assert!(out.contains("164"), "call count present");
            assert!(out.contains("1.9M"), "input tokens compact");
            assert!(out.contains("31.2k"), "output tokens compact");
            assert!(out.contains("8 min 12 s"), "summed request time");
            assert!(out.contains("6 min 3 s"), "wall-clock generation time");
        }
        // No stats line when there were no calls (e.g. dry-run), but the credit link stays.
        d.meta.calls = 0;
        let md = d.to_markdown();
        assert!(md.contains("crawler.siteone.io"));
        assert!(!md.contains("LLM:"), "no stats line without calls");
    }
