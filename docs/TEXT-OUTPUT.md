...
| Insecure cipher suites    | None                                                                                  |
| Without forward secrecy   | None                                                                                  |
...
*   **Text:** The corresponding value for the information type. Supported protocols lists every version from SSLv3 to TLS 1.3 that the server accepts (legacy/unsafe versions are also flagged in the summary). Insecure cipher suites lists every NULL, EXPORT, anonymous, RC4, DES or 3DES suite the server accepts on any protocol up to TLS 1.2, with the protocols that accepted it; Without forward secrecy lists accepted static-RSA suites (AES, CAMELLIA, ARIA, SEED; the first one found is enough). Long lists show the first suites and "… (+k more)" (the summary and the JSON output keep the full list); a note in front says when the probe was cut short or a server gave no clear answer (silence, a cut-off or malformed reply, a cut-off alert included), and then no OK is reported. A fatal alert counts as a refusal of the offered suites, and so does a connection closed or reset before any answer arrived (Windows SChannel/IIS and some load balancers answer this way when no suite is shared; sslyze and testssl.sh count it the same way); a warning alert before the ServerHello is skipped. Trust reports whether the certificate chain validates against the system CA store.
...
*   **Negative (Critical/Warning):** expired or not‑yet‑valid certificate; certificate expiring within 14 days (Warning); hostname mismatch; self‑signed / untrusted root; weak signature algorithm anywhere in the chain (SHA‑1/MD5, incl. weak intermediates); weak public key (RSA < 2048‑bit, EC < 256‑bit, DSA); supported obsolete protocols (SSLv3 / TLS 1.0 / TLS 1.1); insecure cipher suites (NULL, EXPORT, anonymous, RC4, DES, 3DES) accepted on any protocol up to TLS 1.2; missing TLS 1.2/1.3; and a failed TLS handshake (the server may only offer obsolete protocols or cipher suites such as RC4, 3DES or weak Diffie‑Hellman, which this crawler refuses to speak — the insecure suites it accepts are still listed, also when no page could be crawled because of it).
*   **Informational (Notice):** missing Common Name (CN); empty Subject (identity provided only via SANs); cipher suites without forward secrecy (static RSA key exchange) — no score impact.
*   **Positive (OK):** trusted chain; strong signature algorithm (SHA‑256+/EdDSA); strong public key (RSA ≥ 2048 / EC ≥ 256); only modern TLS protocols (no SSLv3 / TLS 1.0 / TLS 1.1); TLS 1.3 supported; no insecure cipher suites accepted.

Cipher suites are enumerated with a handful of extra handshakes: for each protocol the crawler offers the weak suites in a ClientHello, records the suite the server picks and repeats without it — at most 20 handshakes and 10 seconds per host. When the `--accept-invalid-certs` (`-aic`) flag is used, certificate problems (untrusted/expired/weak) are reported as Warnings instead of Criticals, since the operator has explicitly opted to accept them. Out of scope (require infrastructure the crawler intentionally avoids): certificate **revocation** (OCSP/CRL), Certificate Transparency (SCT) and Diffie‑Hellman parameter strength.
